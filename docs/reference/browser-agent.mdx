...
Access the underlying `WebHarness` instance for advanced browser inspection capabilities.

<RequestExample>
```typescript Advanced Inspection
const agent = await startBrowserAgent();
...
harness.clearConsoleLogs();
harness.clearNetworkRequests();
```
</RequestExample>

**Available WebHarness Methods:**

- `getPageHTML()` - Returns full HTML content of the current page
- `getAccessibilityTree()` - Returns the accessibility tree with interactive elements
- `getConsoleLogs(clear?: boolean)` - Returns captured console messages (optionally clears buffer)
- `getNetworkRequests(clear?: boolean)` - Returns captured network requests (optionally clears buffer)
- `clearConsoleLogs()` - Clears the console log buffer
- `clearNetworkRequests()` - Clears the network request buffer
- `isConsoleMonitoringEnabled()` - Check if console monitoring is enabled
- `isNetworkMonitoringEnabled()` - Check if network monitoring is enabled

<Note>
Configure buffer limits via `BrowserConnectorOptions`:
```typescript
const agent = await startBrowserAgent({
  connector: new BrowserConnector({
    consoleLogLimit: 1000,      // default: 500, minimum: 10
    networkRequestLimit: 200    // default: 100, minimum: 10
  })
});
```
</Note>
